...
void TienLenPlayState::onEnter()
{
    ...
    auto currentColor = static_cast<TienLenRule::Color>(game._currentColor);
    auto currentValue = static_cast<TienLenRule::Value>(game._currentValue);
    ...
    if (game._currentValue == TienLenRule::Value::WILD_DRAW_FOUR && game._plus4Stacked)
    {
    ...
    if (canPlayCard)
        ...
        {
            auto cardColor = static_cast<TienLenRule::Color>(card->getValue("color"));
            auto cardValue    = static_cast<TienLenRule::Value>(card->getValue("value"));
            if ((cardColor == currentColor && game._plus2StackCount == 0) || cardColor == TienLenRule::Color::WILD || cardValue == currentValue)
            {
                card->unlockInput();
                hasPlayableCard = true;
            }
        }
    if (!hasPlayableCard && (currentValue == TienLenRule::Value::DRAW_TWO && game._plus2StackCount > 0))
    {
...
void TienLenPlayState::onWebSocketMessage(EventWebSocket* event)
{
    ...
    if (cmd == "player_response")
    {
        ...
        {
            game._currentColor = static_cast<TienLenRule::Color>(payload["new_current_color"]);
        }

        if (payload.contains("new_current_value"))
        {
            game._currentValue = static_cast<TienLenRule::Value>(payload["new_current_value"]);
        }
...
void TienLenPlayState::onCardClicked(EventCard* event) {
    ...
    auto cardColor = static_cast<TienLenRule::Color>(clickedCard->getValue("color"));
    auto cardValue = static_cast<TienLenRule::Value>(clickedCard->getValue("value"));
    ...
    if (cardColor == TienLenRule::Color::WILD)
    {
        ...
        for (auto& button : game._colorButtons)
        {
            ...
            static_cast<ax::ui::Button*>(button)->setTitleText(magic_enum::enum_name(static_cast<TienLenRule::Color>(i)));
            game._colorButtons.at(i)->addClickEventListener([this, &game, i](ax::Object* sender) {
                game._currentColor = static_cast<TienLenRule::Color>(i);
